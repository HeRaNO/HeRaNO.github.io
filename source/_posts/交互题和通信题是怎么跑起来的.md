...
Polygon 上只支持选手程序运行恰好两次。如果要在 Polygon 上造通信，必须勾选 `Is problem interactive`，在 `Advanced` 页面勾选 `Run solutions twice`。`Interactive second invocation` 是指第二次运行是不是按交互题运行，因为第一次运行一定是按交互题运行的。
